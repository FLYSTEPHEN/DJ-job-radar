...
def select_website_scraped(page_url):
    try:
        ...
        return []
def scrape_all_pages(base_url,starting_index,step):
    ...
    while True: 
        ...
        jobs = select_website_scraped(page_url)
...
for i in websites:
    all_jobs=[] 
    #we will use all_jobs to do duplicate checking in functions, camparing one of whose items with a list
    #so that we define all_jobs firstly
    scrape_all_pages(i["base_url"],i["starting_index"],i["step"])
